...
words_bing <- word_tibble_freq %>% inner_join(get_sentiments("bing"), by="word") %>%
  select(word, sentiment, n) %>% mutate(word = reorder(word, n))
head(words_bing, 10)
...
From the bar charts above, **love** is a sentiment that is expressed far more frequently than other sentiments in lyrics. If we don't take **love** into consideration, we will find that the top ten sentiments which are used most frequently in lyrics are all negative sentiments. The top ten positive sentiments can reflect people's hopes and pursuit of better life. While the top ten negative sentiments represent the dark side of life. I think the reason why negative sentiments are used more frequently is that people use music to release their bad emotions and face their lives with a positive attitude. At the same time, negative sentiments in lyrics can strike a chord with people. In my personal experience, I prefer listening to songs with negative emotions when I am alone.
...
**Love** is a sentiment that is expressed far more frequently than others in lyrics. Negative sentimens are
used far more frequently than positve sentiments in lyrics. 

I hope this report can help you get a simple understanding of the changing trend of lyrics and the sentiments contained in lyrics.
